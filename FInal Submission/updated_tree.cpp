...
void find_pivots( vector<struct data> &dataset , vector<struct data> &pivots ) 
{
    ...
    int idx_piv1 = distances_data[0].first , idx_piv2 = distances_data[1].first ; 
    pivots.push_back( dataset[idx_piv1] ) ;
    pivots.push_back( dataset[idx_piv2] ) ;
...
groups create_sub_groups( vector<struct data> pivots , vector<struct data> &dataset ){
...
void recursive_partition( vector<struct data> &dataset , node** root , point nill )
{
    if ( (dataset).size() > 2 )
    {
        ...
        insert(  pivots[0] , root , nill ) ; 
        insert(  pivots[1] , root , nill ) ; 

        groups sub_groups = create_sub_groups( pivots , dataset ) ;

        recursive_partition( sub_groups[0] , root , nill ) ; 
        recursive_partition( sub_groups[1] , root , nill ) ;  
    }
}
...
//     cout << dist << endl ; 
    ...
    recursive_partition( dataset , &root , nill) ; 
    ...
    point neigh = neighbour( query , root , euclid_dist );
